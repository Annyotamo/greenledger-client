...
export function Category7Table({
    entries,
    isLoading,
    onViewDetail,
    onEdit,
                ...
                <Table className="w-full table-auto">
                    <TableHeader>
                        <TableRow className="bg-surface-container-low border-b border-outline-variant/60 font-mono text-xs text-on-surface-variant">
                            ...
                            <TableHead className="py-3 px-4">Commute Title & Description</TableHead>
                            <TableHead className="py-3 px-4">Transit Modes & Segments</TableHead>
                            <TableHead className="py-3 px-4">Total Commute Distance</TableHead>
                            <TableHead className="py-3 px-4">Commute Emissions</TableHead>
                        ...
                        {isLoading ? (
                            <TableRow>
                                <TableCell colSpan={7} className="py-12 text-center font-mono text-xs text-on-surface-variant">
                                    Loading Employee Commute records...
                                </TableCell>
                            </TableRow>
                        ) : entries.length === 0 ? (
                            <TableRow>
                                <TableCell colSpan={7} className="py-12 text-center font-mono text-xs text-on-surface-variant">
                                    No Employee Commute activity logged yet. Click &quot;Log Employee Commute Activity&quot; to begin.
                                </TableCell>
                            </TableRow>
                        ) : (
                            entries.map((entry) => {
                                        ...
                                        <TableCell className="py-3 px-4">
                                            <div className="flex items-center gap-1.5 font-bold text-primary">
                                                <span className="flex items-center gap-1 rounded bg-emerald-50 px-1.5 py-0.5 text-[10px] text-emerald-700 border border-emerald-200">
                                                    <MaterialIcon name="commute" size="xs" /> Commute
                                                </span>
                                            </div>
                                            <div className="text-[10px] text-on-surface-variant mt-1">
                                                {entry.trips.length} commute leg{entry.trips.length === 1 ? "" : "s"}
                                            </div>
                                        </TableCell>
