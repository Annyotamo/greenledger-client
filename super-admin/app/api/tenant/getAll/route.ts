...
export async function GET() {
    try {
        ...
        const url = apiUrl("/tenant/getAll");
        ...
        if (axios.isAxiosError(err)) {
            ...
            return NextResponse.json(
                { message: backendMessage ?? "Get tenants failed.", details: err.response?.data },
                { status: err.response?.status ?? 500 },
            );
        }

        const message =
            typeof err === "object" && err && "message" in err ? String((err as any).message) : "Get tenants failed.";
