...
export function Scope3DashboardView() {
    ...
    // Only display categories whose APIs have been fully implemented
    const activeCategories = SCOPE3_CATEGORIES.filter((cat) => cat.isImplemented);
    ...
    return (
        <div className="relative mx-auto max-w-[1400px] space-y-6 pb-12">
                ...
                <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
                    <div className="space-y-2">
                        ...
                        <p className="font-mono text-xs text-on-surface-variant max-w-2xl">
                            Active Scope 3 value chain accounting across implemented upstream and downstream categories under the GHG Protocol.
                        </p>
                    </div>
                ...
                <div className="mt-6 border-t border-outline-variant/40 pt-4">
                    <div className="flex items-center justify-between font-mono text-xs font-medium text-primary mb-1.5">
                        <span className="flex items-center gap-1.5 text-secondary font-bold">
                            ...
                            Upstream (Cat 1–7): {SCOPE3_SUMMARY.upstreamEmissionsTco2e.toLocaleString()} tCO₂e ({SCOPE3_SUMMARY.upstreamSharePercent}%)
                        </span>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <Card className="p-5 border-outline-variant/60">
                    <div className="flex items-center justify-between text-on-surface-variant">
                        <span className="font-mono text-xs font-semibold uppercase tracking-wider">Total Active Footprint</span>
                        <MaterialIcon name="hub" className="text-secondary" />
                    </div>
                    <div className="mt-2 space-y-1">
                        <p className="font-mono text-headline-md font-bold text-primary">
                            {SCOPE3_SUMMARY.totalEmissionsTco2e.toLocaleString()} <span className="text-xs font-normal">tCO₂e</span>
                        </p>
                ...
                <Card className="p-5 border-outline-variant/60">
                    ...
                    <div className="mt-2 space-y-1">
                        <p className="font-mono text-headline-md font-bold text-primary">
                            {SCOPE3_SUMMARY.upstreamEmissionsTco2e.toLocaleString()} <span className="text-xs font-normal">tCO₂e</span>
                        </p>
                        <p className="font-mono text-[11px] text-on-surface-variant">
                            7 Categories Active (Cat 1–7)
                        </p>
                    </div>
                </Card>
                ...
                <Card className="p-5 border-outline-variant/60">
                    ...
                    <div className="mt-2 space-y-1">
                        ...
                        <p className="font-mono text-[11px] text-on-surface-variant">
                            ${(SCOPE3_SUMMARY.verifiedSpendUsd / 1000000).toFixed(1)}M Mapped Spend
                        </p>
                    </div>
                </Card>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-outline-variant/50 pb-3">
                <div className="flex items-center gap-2">
                    <span className="font-mono text-xs font-bold text-primary uppercase">Active Categories ({activeCategories.length}):</span>
                    <div className="flex items-center rounded-lg bg-surface-container-high p-1">
                        <button
                            ...
                            All Active ({activeCategories.length})
                        </button>
                        <button
                            type="button"
            ...
            {/* Implemented Category Cards Matrix */}
            <motion.div
                ...
                <div className="col-span-12 lg:col-span-7">
                    <Card className="h-full p-6 border-outline-variant/60">
                        <CardHeader className="px-0 pt-0 pb-4 border-b border-outline-variant/40 flex flex-row items-center justify-between">
                            ...
                            <span className="font-mono text-xs text-on-surface-variant">tCO₂e / Month</span>
                        </CardHeader>
                        <CardBody className="px-0 pt-4 pb-0 space-y-4">
                            ...
                            <div className="flex items-center justify-center gap-6 font-mono text-xs font-semibold pt-2 border-t border-outline-variant/30">
                                <span className="flex items-center gap-1.5">
                                    <span className="h-3 w-3 rounded-sm bg-secondary" /> Upstream Supply Chain (Cat 1–7)
                                </span>
                                <span className="flex items-center gap-1.5">
                                    <span className="h-3 w-3 rounded-sm bg-tertiary-container" /> Investments (Cat 15)
                                </span>
                            </div>
                        </CardBody>
                    </Card>
                ...
                <div className="col-span-12 lg:col-span-5">
                    <Card className="h-full p-6 border-outline-variant/60">
                        <CardHeader className="px-0 pt-0 pb-4 border-b border-outline-variant/40 flex flex-row items-center justify-between">
                            ...
                            <span className="font-mono text-xs text-on-surface-variant">Top Tier Activities</span>
                        </CardHeader>
                    ...
                    <div className="flex items-baseline justify-between">
                        <div>
                            <p className="font-mono text-headline-sm font-bold text-primary">
                                {category.emissionsTco2e.toLocaleString()} <span className="text-xs font-normal">tCO₂e</span>
                            </p>
                            <p className="font-mono text-[10px] text-on-surface-variant">
                                {category.sharePercent}% of Scope 3
                            </p>
                        </div>
                        <span
                            className={cn(
