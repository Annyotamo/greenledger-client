...
interface Category4TableProps {
    ...
    isLoading: boolean;
    onViewDetail: (entry: Category4SpendEntry) => void;
    onEdit: (entry: Category4SpendEntry) => void;
    onAmend: (entry: Category4SpendEntry) => void;
    onSubmitEntry: (id: string) => void;
    onVerifyEntry: (id: string) => void;
    onRejectEntry: (entry: Category4SpendEntry) => void;
                ...
                <Table className="w-full table-auto">
                    <TableHeader>
                        <TableRow className="bg-surface-container-low border-b border-outline-variant/60 font-mono text-xs text-on-surface-variant">
                            ...
                            <TableHead className="py-3 px-4">NAICS Freight Commodity & Factor</TableHead>
                            <TableHead className="py-3 px-4">Spend (INR / USD)</TableHead>
                            <TableHead className="py-3 px-4">Calculated T&D Emissions</TableHead>
                        ...
                        {isLoading ? (
                            <TableRow>
                                <TableCell colSpan={6} className="py-12 text-center font-mono text-xs text-on-surface-variant">
                                    Loading Category 4 freight transport spend entries...
                                </TableCell>
                            </TableRow>
                        ) : entries.length === 0 ? (
                            <TableRow>
                                <TableCell colSpan={6} className="py-12 text-center font-mono text-xs text-on-surface-variant">
                                    No Upstream Transportation spend recorded yet. Click &quot;Log Freight Spend Entry&quot; to begin.
                                </TableCell>
                            </TableRow>
                        ) : (
                            entries.map((entry) => {
                                    ...
                                    <TableRow
                                        ...
                                        <TableCell className="py-3 px-4">
                                            ...
                                            <div className="text-[11px] text-on-surface-variant">
                                                {entry.spendDate ? format(new Date(entry.spendDate), "MMM d, yyyy") : "N/A"}
                                            </div>
                                            {entry.facilityName && (
                                        ...
                                        <TableCell className="py-3 px-4">
                                            ...
                                            <div className="text-[10px] text-secondary font-medium">
                                                NAICS {entry.factor?.naicsCode || "484110"} • {entry.factor?.kgCo2ePerUsdWithMargins || 0.415} kg/USD
                                            </div>
                                        </TableCell>

                                        <TableCell className="py-3 px-4">
                                            <div className="font-bold text-primary">
                                                ₹{entry.spendInInr.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                            </div>
