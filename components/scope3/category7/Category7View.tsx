...
import { Category7Summary } from "./Category7Summary";
import { Category7Table } from "./Category7Table";
import { Category7FormModal } from "./Category7FormModal";
import { Category7DetailModal } from "./Category7DetailModal";
import { Category7RejectModal } from "./Category7RejectModal";
import {
...
export function Category7View() {
    ...
    const deleteMutation = useDeleteTravelActivity("EMPLOYEE_COMMUTING");
    const submitMutation = useSubmitTravelActivity("EMPLOYEE_COMMUTING");
    const verifyMutation = useVerifyTravelActivity("EMPLOYEE_COMMUTING");
    const rejectMutation = useRejectTravelActivity("EMPLOYEE_COMMUTING");
    ...
    async function handleConfirmReject(reason: string) {
        ...
        try {
            ...
            showNotify("success", "Employee commute record rejected.");
    ...
    return (
        <div className="relative mx-auto max-w-[1400px] space-y-6 pb-12">
            ...
            {/* Summary KPI Cards */}
            <Category7Summary entries={travelQuery.data ?? []} summary={summaryQuery.data} />
            ...
            <Category7Table
                ...
                onSubmitEntry={async (id) => {
                    try {
                        ...
                        showNotify("success", "Employee commute entry submitted for review.");
                    } catch (err: unknown) {
                ...
                onVerifyEntry={async (id) => {
                    try {
                        ...
                        showNotify("success", "Employee commute entry verified.");
                    } catch (err: unknown) {
                ...
                onDeleteEntry={async (id) => {
                    if (!confirm("Are you sure you want to delete this employee commute record?")) return;
                    try {
                        await deleteMutation.mutateAsync(id);
                        showNotify("success", "Employee commute entry deleted.");
                    } catch (err: unknown) {
            ...
            {/* Form Modal */}
            <Category7FormModal
                key={`cat7-${selectedEntry?.id ?? "new"}-${formMode}-${isFormOpen}`}
                isOpen={isFormOpen}
                mode={formMode}
            ...
            {/* Detail Modal */}
            <Category7DetailModal entry={detailEntry} onClose={() => setDetailEntry(null)} />

            {/* Reject Modal */}
            <Category7RejectModal
                isOpen={isRejectOpen}
                onClose={() => {
                    setIsRejectOpen(false);
                    setRejectTarget(null);
